...
from haystack.components.converters.jq_json import JSONToDocument


class TestJSONToDocument:
    def test_run(self, test_files_path, caplog):
        """Happy path test for standard and JSON lines JSON"""
        ...
        expected_meta_lines = {
            ...
            "share": None,
            "seq_no": 1,
            "file_path": "sample_json_lines.json",
            "jq_schema": "del(.share)",
        }
        ...
        with caplog.at_level(logging.WARNING):
            output_dict = converter.run(
                sources=file_path,
                jq_data_schema=jq_data_schema,
                metadata_func=metadata_func,
                metadata={"dataset": "nobel data set"},
                json_lines=False,
            )
        ...
        # Normalize for whitespace
        expected_content = {
            key: " ".join(value.split()) if isinstance(value, str) else value for key, value in expected_content.items()
        }
        doc_dict = {
            key: " ".join(value.split()) if isinstance(value, str) else value for key, value in doc_dict.items()
        }
        expected_meta = {
            key: " ".join(value.split()) if isinstance(value, str) else value for key, value in expected_meta.items()
        }
        meta = {key: " ".join(value.split()) if isinstance(value, str) else value for key, value in meta.items()}
        ...
        converter_lines = JSONToDocument()

        with caplog.at_level(logging.WARNING):
            output_dict = converter_lines.run(
                sources=file_path_lines,
                jq_data_schema=jq_data_schema_lines,
                metadata_func=metadata_func,
                metadata={"dataset": "nobel data set"},
                json_lines=True,
            )
        ...
        expected_content_lines = {
            ...
            for key, value in expected_content_lines.items()
        }
        doc_dict = {
            key: " ".join(value.split()) if isinstance(value, str) else value for key, value in doc_dict.items()
        }
        expected_meta_lines = {
            key: " ".join(value.split()) if isinstance(value, str) else value
            for key, value in expected_meta_lines.items()
        }
        meta = {key: " ".join(value.split()) if isinstance(value, str) else value for key, value in meta.items()}
    ...
    def test_run_error_handling(self, test_files_path, caplog):
        ...
        converter_error = JSONToDocument()
        with caplog.at_level(logging.WARNING):
            output = converter_error.run(  # noqa: F841
                sources=file_path_error, jq_data_schema="", metadata_func=None, metadata=None, json_lines=False
            )
